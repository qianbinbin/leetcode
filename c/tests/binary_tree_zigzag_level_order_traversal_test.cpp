...
TEST(binary_tree_zigzag_level_order_traversal_test, zigzagLevelOrder_103_1) {
    ...
    std::vector<std::vector<int>> expected1{{3},
                                            {20, 9},
                                            {15, 7}};
    int **actual1 = zigzagLevelOrder_103_1(root1, &returnSize1,
                                           &returnColumnSizes1);
    ...
    int **actual2 = zigzagLevelOrder_103_1(root2, &returnSize2,
                                           &returnColumnSizes2);
    ...
    int **actual3 = zigzagLevelOrder_103_1(root3, &returnSize3,
                                           &returnColumnSizes3);
...
TEST(binary_tree_zigzag_level_order_traversal_test, zigzagLevelOrder_103_2) {
    ...
    std::vector<std::vector<int>> expected1{{3},
                                            {20, 9},
                                            {15, 7}};
    int **actual1 = zigzagLevelOrder_103_2(root1, &returnSize1,
                                           &returnColumnSizes1);
    ASSERT_EQ(expected1.size(), returnSize1);
    for (int i = 0; i < returnSize1; ++i) {
        EXPECT_EQ(expected1[i], std::vector<int>(actual1[i], actual1[i] +
    ...
    int *returnColumnSizes2 = NULL;
    std::vector<std::vector<int>> expected2{{1}};
    int **actual2 = zigzagLevelOrder_103_2(root2, &returnSize2,
                                           &returnColumnSizes2);
    ASSERT_EQ(expected2.size(), returnSize2);
    for (int i = 0; i < returnSize2; ++i) {
        EXPECT_EQ(expected2[i], std::vector<int>(actual2[i], actual2[i] +
    ...
    int *returnColumnSizes3 = NULL;
    std::vector<std::vector<int>> expected3;
    int **actual3 = zigzagLevelOrder_103_2(root3, &returnSize3,
                                           &returnColumnSizes3);
    ASSERT_EQ(expected3.size(), returnSize3);
    for (int i = 0; i < returnSize3; ++i) {
        EXPECT_EQ(expected3[i], std::vector<int>(actual3[i], actual3[i] +
