...
generate_trees(int start, int end, int *size, int *tree_counts) {
    const int capacity = tree_counts[end - start];
    struct TreeNode **ret = (struct TreeNode **) malloc(
    ...
    *size = 0;
    ...
    for (int val = start; val < end; ++val) {
        ...
        struct TreeNode **left_trees = generate_trees(start, val, &left_size,
                                                      tree_counts),
                **right_trees = generate_trees(val + 1, end, &right_size,
                                               tree_counts);
        for (int i = 0; i < left_size; ++i) {
            for (int j = 0; j < right_size; ++j) {
                struct TreeNode *root = (struct TreeNode *) malloc(
