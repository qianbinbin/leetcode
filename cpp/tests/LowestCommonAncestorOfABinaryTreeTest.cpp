...
TEST(LowestCommonAncestorOfABinaryTree, Solution236_4) {
  const auto Root1 = newTree({3, 5, 1, 6, 2, 0, 8, NTNode, NTNode, 7, 4});
  const auto &P1 = Root1->left, &Q1 = Root1->right, &Expected1 = Root1;
  EXPECT_EQ(Expected1, Solution236_4().lowestCommonAncestor(Root1, P1, Q1));
  deleteTree(Root1);

  const auto Root2 = newTree({3, 5, 1, 6, 2, 0, 8, NTNode, NTNode, 7, 4});
  const auto &P2 = Root2->left, &Q2 = Root2->left->right->right,
             &Expected2 = P2;
  EXPECT_EQ(Expected2, Solution236_4().lowestCommonAncestor(Root2, P2, Q2));
  deleteTree(Root2);
}
