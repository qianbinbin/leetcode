...
// range [-2^31, 2^31 - 1], then clamp the integer so that it remains in the
// range. Specifically, integers less than -2^31 should be clamped to -2^31, and
// integers greater than 2^31 - 1 should be clamped to 2^31 - 1. Return the
...
// current reader position. Step 1: "42" (no characters read because there is no
// leading whitespace)
//         ^
...
#ifndef LEETCODECPP_STRINGTOINTEGERATOI_H
#define LEETCODECPP_STRINGTOINTEGERATOI_H
...
#endif // LEETCODECPP_STRINGTOINTEGERATOI_H
