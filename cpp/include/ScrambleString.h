...
//    the string is s, divide it to x and y where s = x + y. Randomly decide to
//    swap the two substrings or to keep them in the same order. i.e., after
//    this step, s may become s = x + y or s = y + x. Apply step 1 recursively
//    on each of the two substrings x and y.
//
...
// recursively on both substrings. divide at ranom index each of them. "g/r /
...
// which is s2. As there is one possible scenario that led s1 to be scrambled to
// s2, we return true.
//
// Example 2:
//
...
#ifndef LEETCODECPP_SCRAMBLESTRING_H
#define LEETCODECPP_SCRAMBLESTRING_H
