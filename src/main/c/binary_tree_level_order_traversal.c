...
void traverse(struct TreeNode *root, int **values, int level, int *sizes, int *capacities) {
    if (root == NULL) return;

    append_array(&values[level], &sizes[level], &capacities[level], root->val);
    traverse(root->left, values, level + 1, sizes, capacities);
    traverse(root->right, values, level + 1, sizes, capacities);
}

int **levelOrder_102(struct TreeNode *root, int **columnSizes, int *returnSize) {
    ...
    *columnSizes = (int *) malloc(*returnSize * sizeof(int));
    ...
    traverse(root, ret, 0, *columnSizes, capacities);
